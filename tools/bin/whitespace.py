...
def main( ):
    ...
    for srcfile in locate(file_patterns, file_ignore_patterns, dir_ignore):
        ...
        if ns.wscmd == valid_commands[2]:
            '''first, generate a list of non-compliant files for the 
            users benefit'''
            Popen(["uncrustify", "-q", "-c", 
                    uncrustify_config, srcfile], stdout=PIPE).communicate()[0]
...
def get_parser( ):
    parser = argparse.ArgumentParser(
        ...
        Script will locate the uncrustify config file, or optionally, 
        the user may specify one.  
